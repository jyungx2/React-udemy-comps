...
const NavigationProvider = ({ children }) => {
  ...
  // 1. 📕URL을 변경하기 위해 pushState를 호출📕하고,
  // 2. 📒컴포넌트를 리렌더링📒하기 위해 React 상태(currentPath)도 직접 업데이트
  // 👉 pushState는 주소창(URL)만 바꾸는 역할, popstate는 유저가 뒤로/앞으로 버튼 클릭했을 때 발생하는 이벤트로서,  📕pushState()로 url을 바꿈📕과 동시에 popstate가 발생했을 때(addEventListener), 📒setCurrentPath()로 상태를 업데이트해주어 실제 화면에 컴포넌트를 리렌더링📒 해줘야 한다.
  // 이때, 💥pushState만 호출하면 컴포넌트가 리렌더링되지 않는다💥 (= pushstate가 Popstate 이벤트를 불러오지는 않는다!) React에서 주소창에 따른 📒UI 변경은 상태 변화에 의존📒하므로, URL 변경과 동시에 React 상태(currentPath)를 업데이트해야 컴포넌트를 다시 렌더링하여 사용자에게 변경된 화면을 보여줄 수 있기 때문에 📒setter function📒 또한 불러줘야 한다.
  // ex) to = '/accordion'
  const navigate = (to) => {
